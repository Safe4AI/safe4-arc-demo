...
def _wait_for_receipt(client: httpx.Client, tx_hash: str, timeout: float = 90.0) -> dict:
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        receipt = _rpc(client, "eth_getTransactionReceipt", [tx_hash])
        if receipt:
            return receipt
        time.sleep(2.0)
    raise RuntimeError(f"receipt not observed within {timeout:.0f}s for {tx_hash}")
