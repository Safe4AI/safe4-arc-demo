...
| Circle Agent Stack executed a fresh 0.01 testnet USDC transfer on Arc after Safe4 returned ALLOWED. | Deck 5–7; video 2:20 | Transaction `0x648ef14e4da7c6bfecce0017d19280ed51fb12635bea94712de926d9f967752c`; block `54014886`; `docs/hackathon/ARC_IMPLEMENTATION_CHALLENGES.md` | Testnet only. The live run stopped after broadcast when the old EOA-shaped verifier rejected the ERC-4337 envelope; the corrected verifier then confirmed the exact receipt without another broadcast. |
| The fresh Circle Agent Wallet receipt is RPC-verified through its ERC-4337 EntryPoint event and exact Arc native-USDC transfer event. | Deck 7; video demo | `verify_circle_agent_wallet_payloads`; `CircleAgentWalletVerifierTests`; no-broadcast `circle-rpc-replay` output | Arc exposes native USDC at 18-decimal RPC precision and the ERC-20 interface at 6 decimals over one balance; the verifier normalizes explicitly. |
